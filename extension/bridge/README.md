...
2. Generates ISO 8601 timestamp: `datetime.now().isoformat()`
3. Formats conversation as:
   ```
   User: <user_message>
...
4. Creates metadata:
   ...
   ```
5. Calls `cognee.add(conversation, dataset_name="copilot_chat", metadata=metadata)`
6. Calls `cognee.cognify()` to build/update knowledge graph with ontology-based entity extraction
...
**Usage**:
```bash
python retrieve.py <workspace_path> <query> [max_results] [max_tokens] [recency_weight] [importance_weight]
```

**Arguments**:
- `workspace_path`: Absolute path to workspace root
- `query`: Search query string (user's question or topic)
- `max_results`: (Optional) Maximum number of results, defaults to 3
- `max_tokens`: (Optional) Maximum total tokens across results, defaults to 2000
- `recency_weight`: (Optional) Weight for recency score (0.0-1.0), defaults to 0.3
- `importance_weight`: (Optional) Weight for importance score (0.0-1.0), defaults to 0.2
...
3. For each result, calculates weighted final score:
   ...
   ```
4. Sorts results by final_score (descending)
5. Applies dual limits:
   - Stops at `max_results` count OR
   - Stops when `total_tokens > max_tokens`
   - (Whichever limit is reached first)
6. Token estimation: `len(text.split())` for rough word-based count
...
2. **Question**: User's query or request
3. **Answer**: Assistant's response
4. **Topic**: Subject area (e.g., "async programming", "API design")
5. **Concept**: Technical concept (e.g., "event loop", "dependency injection")
6. **Problem**: Issue described by user
7. **Solution**: Proposed solution or approach
8. **Decision**: Decision made (e.g., "use FastAPI", "avoid threads")

### Relationships (12)
1. **ASKS**: User → Question
2. **HAS_TOPIC**: Question → Topic
3. **MENTIONS**: Question → Concept
4. **DESCRIBES**: Question → Problem
5. **ADDRESSES**: Answer → Question
6. **PROPOSES**: Answer → Solution
7. **EXPLAINS**: Answer → Concept
8. **SOLVES**: Solution → Problem
9. **RELATED_TO**: Topic ↔ Topic
10. **FOLLOWS_UP**: Question → Question (conversation threads)
11. **IMPACTS**: Decision → Topic
12. **PREREQUISITE_FOR**: Concept → Concept (dependencies)
...
2. Initialize Cognee for workspace:
   ```bash
   python bridge/init.py /path/to/workspace
   ```

3. Cognee stores data in `workspace/.cognee/` directory (created automatically)
